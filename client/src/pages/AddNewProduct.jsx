...
const AddNewProduct = () => {
  ...
  const handleProductImages = (evt) => {
    ...
    setprodcutImages((prev) => ({
      ...prev,
      [targetedInputName]: finalImageUrl,
    }));
  ...
  const apiCall = async (evt) => {
    ...
    const response = await axios.post(
      "http://localhost:3000/api/v1/product/create-prodcut",
      finalData,
      ...
      <div className="addProdcut">
        ...
        >
          <div className="partOne w-[50%] border-stone-800 border-[1px] p-5 rounded-lg">
            ...
            <input
              type="submit"
              className="h-[50px] border-none w-full bg-green-600 mt-5 rounded-md cursor-pointer "
              required={true}
            />
          </div>
          <div className="partTwo w-[50%] border-stone-800 border-[1px] p-5 rounded-lg">
            <div className="inputPart">
              <h2 className="text-2xl mb-5">
                Select images for Your{" "}
                <span className="text-yellow-500">Prodcut</span>
              </h2>
              <label className="text-stone-400">Select Main Image</label>
              <input
                onChange={(evt) => handleProductImages(evt)}
                name="mainImage"
                type="file"
                className="inputStyling  mb-4"
                required={true}
              />
              {/* priview all images  */}
              <div className="allPriviewImages">
                <h2 className="text-2xl mb-5">
                  Privew The All <span className="text-yellow-500">Images</span>
                </h2>
                {/* main images  */}
                <div className="allImages flex gap-2">
                  <img
                    className="h-[310px] w-[220px] object-cover rounded-md"
                    src={prodcutImages?.mainImage || "/productPlaceholder.png"}
                    alt=""
                  />
                  {/* others four  */}
                  <div className="fourImg w-[280px] h-[300px] flex flex-wrap gap-2 ">
                    <img
                      className="h-[150px] w-[110px] object-cover rounded-md"
                      src={prodcutImages?.imageOne || "/productPlaceholder.png"}
                      alt=""
                    />
                    <img
                      className="h-[150px] w-[110px] object-cover rounded-md"
                      src={prodcutImages?.imageTwo || "/productPlaceholder.png"}
                      alt=""
                    />
                    <img
                      className="h-[150px] w-[110px] object-cover rounded-md"
                      src={
                        prodcutImages?.imageThree || "/productPlaceholder.png"
                      }
                      alt=""
                    />
                    <img
                      className="h-[150px] w-[110px] object-cover rounded-md"
                      src={
                        prodcutImages?.imageFour || "/productPlaceholder.png"
                      }
                      alt=""
                    />
                  </div>
