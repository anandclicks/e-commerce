...
const ProductSpecificPage = () => {
  ...
  useEffect(() => {
    const apiCall = async () => {
      const response = await axios.get(
        "http://localhost:3000/api/v1/customer/loggedinCustomer",
  ...
  useEffect(() => {
    const apiCall = async () => {
      checking();
      const response = await axios.get(
        `http://localhost:3000/api/v1/product/details/${id}`
      );
      if (response.data.sucess) {
  ...
  const addToCartApiCall = async () => {
    ...
    const response = await axios.post(
      "http://localhost:3000/api/v1/customer/add-to-cart",
      { id },
      { withCredentials: true }
    );
    if (!response.data.sucess) {
  ...
  return (
    <div className="productPageWrapper">
      ...
      {!prodcutDetails && <div>Product not found</div>}
        ...
        <div className="prodctuPage">
          ...
          <div className="leftSide">
            ...
            <div className="ctaButtons mt-10 flex gap-5">
              <div className="buyNow ctaBtns">
                <i className="ri-shopping-cart-fill"></i> Buy now
              </div>
              {!alreadyInCart && (
                <div
                  onClick={() => addToCartApiCall()}
                  className="addToCart ctaBtns"
                >
          ...
          <div className="rightSide">
            ...
            <div className="titleAndPrice">
              ...
              <h3 className="salePrice">
                ...
                <small className="text-red-600 line-through">
                  {prodcutDetails.price}
                </small>
              </h3>
              <div className="mt-3">
                {prodcutDetails.bestSeller && (
                  <div className="bestSellr ">
                    Best Seller<i className="ri-bard-fill"></i>
                  </div>
                )}
              </div>
              <p>
                {prodcutDetails.deleveryCharge > 0 ? (
                  <div className="text-sm mt-2">
                    <i className="ri-truck-fill"></i> Delivery charge: $
                    {prodcutDetails.deleveryCharge}
                  </div>
                ) : (
                  <div className="text-sm mt-2">
                    <i className="ri-truck-fill"></i> Free Delivery
                  </div>
                )}
              </p>
            </div>
            ...
            <div className="productDetails">
              <h3>
                <i className="ri-gift-fill"></i> Product Details
              </h3>
              <div>
                <ul className="text-[16px]">
                  ...
                  <li>
                    ...
                    {100 -
                      Math.round(
                        (prodcutDetails.salePrice / prodcutDetails.price) * 100
                      )}
                    %
                  </li>
                </ul>
            ...
            <div className="sildBy">
              <h3 className="text-[17px]">
                <i className="ri-store-fill"></i> Sold by
              </h3>
              <div className="storeDetails">
                <div className="storeImage flex items-center gap-5 mt-3">
                  <img
                    className="w-[40px] h-[40px] object-cover rounded-full"
                    src={prodcutDetails.seller.profilePicture}
                    alt=""
                  />
                  <div className="nameAndEmail">
                    <p>{prodcutDetails.seller.name}</p>
                    <p className="text-slate-500 leading-3">
                      {prodcutDetails.seller.emailAddress}
                    </p>
                  </div>
                  <div className="originCountry flex gap-2 items-center">
                    <i className="ri-global-line text-lg"></i>{" "}
                    <p>{prodcutDetails.seller.counrtyOrigin}</p>
                  </div>
                  <div className="originCountry flex gap-2 items-center">
                    <i className="ri-customer-service-line"></i>{" "}
                    <p>{prodcutDetails.seller.businessNumber}</p>
                  </div>
                </div>
              </div>
            </div>
