...
const MyCart = () => {
  ...
  useEffect(() => {
    const apiCll = async () => {
      const response = await axios.get(
        "http://localhost:3000/api/v1/customer/get-cart-prodcut",
        { withCredentials: true }
      );
  ...
  const manageCartProduct = async (id) => {
    ...
    if (response.data.sucess) {
      showResponse('Removed From Cart',1500)
    }
    apiCll();
  };
        ...
        <div className="cartLeftSide">
          ...
          {loggedInCustomerCartData?.map((item, index) => (
            <Link
              to={`/product/${item._id}`}
              key={index}
              className="specificProdcutCartOfCartArray mb-3 rounded-md p-1 flex justify-between items-center"
            >
              {/* image and title  */}
              ...
              <div
                onClick={(evt) => {
                  evt.stopPropagation();
                  evt.preventDefault();
                  manageCartProduct(item._id);
                }}
                className="cursor-pointer bg-black p-3 rounded-lg"
              >
                Remove <i className="ri-close-line"></i>
              </div>
            </Link>
          ))}
            ...
            {loggedInCustomerCartData?.map((item, index) => (
              <div className="flex justify-between">
                <div key={index} className="flex w-[60%]">
                  <p className="text-[14px] mt-2  ps-2 px-3 bg-slate-900 py-2 rounded-md">
                    {item.title}
                  </p>
                </div>
                <div key={index} className="w-[20%]">
                  <p className="text-[14px] p-2  mt-2 bg-slate-900 p rounded-md">
                    $ {item.salePrice}
                  </p>
                </div>
              </div>
